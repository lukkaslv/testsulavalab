    ...
    return (
        <div className="absolute inset-0 z-50 bg-white/95 backdrop-blur-md flex flex-col justify-center px-6 animate-in">
            <div className="space-y-4 mb-8 text-center">
                ...
                <p className="text-sm font-medium text-slate-500 leading-relaxed max-w-xs mx-auto text-center">{t.context_check.desc}</p>
            </div>
...
// ---------------------------------------------------------------------------
// 1. PRO DASHBOARD (CLINICAL COMMAND CONSOLE) - For CLINICAL/LAB only
// ---------------------------------------------------------------------------
    ...
    return (
        <div className="flex flex-col h-full bg-slate-950 text-white p-4 space-y-4 animate-in select-none overflow-y-auto no-scrollbar">
            ...
            <header className="flex justify-between items-center border-b border-indigo-900/30 pb-3 shrink-0 px-2">
                ...
                <button 
                    onClick={onLogout}
                    className="w-10 h-10 flex items-center justify-center bg-slate-900 rounded-xl border border-slate-800 text-slate-500 hover:text-red-500 transition-colors"
                >
                    <span className="text-sm">⏻</span>
                </button>
            </header>

            {/* Grid for top modules */}
            <div className="grid grid-cols-2 gap-3 shrink-0">
                ...
                <div className="bg-slate-900 p-3 rounded-2xl border border-white/5 space-y-1.5">
                    ...
                    <div className="text-2xl font-black text-white">
                        {usageStats.isUnlimited ? '∞' : (usageStats.limit - usageStats.used)}
                    </div>
            ...
            {/* Main Action: Terminal Access */}
            <button 
                onClick={() => onSetView('compatibility')}
                className="w-full bg-indigo-600 p-6 rounded-2xl shadow-2xl shadow-indigo-500/20 border border-indigo-500 flex flex-col items-center justify-center gap-2 group active:scale-[0.98] transition-all relative overflow-hidden shrink-0"
            >
            ...
            <div className="space-y-3 shrink-0 pt-2 border-t border-slate-800/50">
                ...
                <div className="grid grid-cols-3 gap-2">
                    ...
                    <button className="p-3 bg-slate-900 border border-slate-800 rounded-xl text-[8px] font-black uppercase text-slate-700 transition-colors flex flex-col items-center gap-1.5 cursor-not-allowed">
                        ...
                        <span className="opacity-50">{ph.data_vault}</span>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div className="space-y-6 animate-in flex flex-col h-full relative">
          ...
          <header className="space-y-3 shrink-0">
            <div className="flex justify-between items-center px-1">
                <div className="flex flex-col">
                    <h2 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2 italic leading-tight">
          ...
          <button 
            ...
            className="w-full py-4 bg-white rounded-2xl flex flex-col items-center justify-center px-4 active:scale-95 transition-all shadow-md border border-slate-200 group"
          >
