    ...
    <section className="space-y-6 animate-in py-4 flex flex-col h-full bg-white">
        <div className="flex justify-between items-center px-1 pb-2 border-b border-indigo-100/50 shrink-0">
            ...
            <div className="flex flex-col items-end">
                 <span className="text-[10px] font-mono text-indigo-600 font-black tracking-widest">{pt.title}</span>
                 <span className="text-[7px] font-mono text-slate-400 uppercase">Supervisor OS v6.8</span>
            </div>
        </div>
        ...
        {!clientResult && (
            <div className="space-y-4 animate-in">
                <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 text-center space-y-4">
                    ...
                    <div>
                        <h3 className="text-sm font-black uppercase text-slate-900">{pt.access_restricted}</h3>
        ...
        {clientResult && interpretation && (
            <div className="bg-slate-950 text-slate-400 p-5 rounded-[2rem] space-y-6 border border-slate-800 shadow-2xl relative animate-in flex-1 overflow-y-auto custom-scrollbar font-mono">
                ...
                <div className="border-b border-slate-800 pb-4 space-y-4">
                    <div className="flex justify-between items-center">
                        <div className="space-y-0.5">
                            <span className="text-[8px] text-emerald-500 font-black uppercase tracking-widest block">{pt.client_id_label}</span>
                            <span className="text-[10px] text-emerald-500 font-mono font-bold tracking-wider">{clientResult.shareCode.substring(0, 12)}...</span>
                        </div>
                ...
                <div className="bg-red-950/20 border border-red-500/30 p-4 rounded-xl space-y-3">
                    <h4 className="text-[9px] font-black uppercase text-red-400 tracking-widest">{pt.signal_quality}</h4>
                ...
                <div className="space-y-4 pt-4 border-t border-slate-800">
                    ...
                    <div className="bg-amber-950/10 p-4 rounded-xl border-l-2 border-amber-500/50">
                        <h4 className="text-[9px] font-black uppercase text-amber-500 tracking-widest mb-2 flex items-center gap-2"><span>⚡</span> {pt.clinical_hypotheses}</h4>
                ...
                <div className="pt-6 border-t border-slate-800 text-center space-y-4">
                     ...
                     <button onClick={() => setClientResult(null)} className="text-[9px] font-black text-red-400 uppercase tracking-widest hover:text-red-300 transition-colors bg-red-950/20 px-4 py-3 rounded-lg border border-red-900/30 w-full">[ {pt.close_session} ]</button>
                </div>
            </div>
        )}
    </section>
