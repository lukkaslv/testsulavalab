...
export const CompatibilityView: React.FC<CompatibilityViewProps> = ({ lang, t, onBack }) => {
  ...
  const interpretation = useMemo(() => {
      ...
      const clinicalData = ClinicalDecoder.decode(clientResult, t);
  ...
  const handleExportMarkdown = () => {
      ...
      const md = `
# ${rpt.title}
**${rpt.client_id}:** ${clientResult.shareCode.substring(0, 12)}
**${rpt.status}:** ${interpretation.priority}

## ${rpt.vitals}
- ${t.domains.foundation}: ${clientResult.state.foundation}%
- ${t.domains.agency}: ${clientResult.state.agency}%
- ${t.domains.money}: ${clientResult.state.resource}%
- ${t.domains.entropy}: ${clientResult.state.entropy}%

## ${rpt.profile}
- ${t.brief_explainer.archetype_label}: ${interpretation.archetype.title}
- ${t.ui.verdict_label}: ${interpretation.verdict.label}
- ${rpt.primary_defense}: ${interpretation.narrative.primaryDefense}
    ...
    <section className={`animate-in py-4 flex flex-col h-full overflow-hidden bg-slate-950 text-slate-400 font-mono`}>
        ...
        <div className={`flex justify-between items-center px-4 pb-4 border-b border-slate-800 shrink-0`}>
            ...
            <div className="flex flex-col items-end">
                 <span className={`text-[10px] font-mono font-black tracking-widest text-emerald-500 italic`}>{pt.terminal_title}</span>
            ...
            {!clientResult && (
                <div className="h-full flex flex-col items-center justify-center p-6 space-y-6 max-w-sm mx-auto animate-in">
                    ...
                    <div className="text-center space-y-3 w-full">
                        <div className="mb-4">
                            <h3 className="text-sm font-black uppercase text-slate-300 tracking-widest">{t.ui.access_restricted}</h3>
                            <p className="text-[9px] text-slate-500 font-medium mt-1 max-w-[200px] mx-auto">
                                {t.ui.enter_code_desc}
                            </p>
                        </div>
                        <input 
                            type="text" 
                ...
                <div className="p-4 space-y-4 animate-in pb-32">
                    ...
                    <div className={`p-4 rounded-2xl border-l-4 shadow-xl flex items-center justify-between ${interpretation.priorityLevel === 'high' ? 'bg-red-950/20 border-red-500' : interpretation.priorityLevel === 'medium' ? 'bg-amber-950/20 border-amber-500' : 'bg-emerald-950/20 border-emerald-500'}`}>
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <span className="text-[7px] font-black text-slate-500 uppercase">{t.ui.priority_label}</span>
                                <p className="text-xs font-black text-white">{interpretation.priority}</p>
                            </div>
                        </div>
                        <button onClick={handleExportMarkdown} className={`p-2 rounded-lg transition-colors ${mdCopySuccess ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/5 hover:bg-white/10'}`} title={pt.export_md_report}>
                          {mdCopySuccess ? '✓' : '📋'}
                        </button>
                    </div>
                    ...
                    <div className="text-center opacity-30 pt-10">
                        <span className="text-[7px] uppercase tracking-[0.8em]">{pt.stream_end}</span>
                    </div>
                </div>
            )}
        </div>
