    ...
    <div className={`space-y-10 pb-32 animate-in px-1 pt-2 font-sans ${isGlitchMode ? 'glitch' : ''}`}>
      ...
      {(result.warnings || []).map((w, i) => (
          <div key={i} className={`p-5 rounded-[2rem] space-y-2 shadow-sm border ${w.severity === 'HIGH' ? 'bg-amber-50 border-amber-200 text-amber-900' : 'bg-blue-50 border-blue-200 text-blue-900'}`}>
              <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest">{t.safety[w.messageKey as keyof typeof t.safety] || w.type}</span>
              </div>
      ...
      <section className="bg-white border border-slate-100 p-6 rounded-[2.5rem] space-y-4 shadow-sm relative overflow-hidden">
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
              <span className="text-[8px] font-black uppercase tracking-widest text-indigo-400 block mb-1">{currentLang === 'ru' ? 'ФОКУС СЕССИИ' : 'სესიის ფოკუსი'}</span>
        ...
        <div className="relative z-10 space-y-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex flex-col items-end">
                  <span className="text-[8px] font-mono font-bold text-slate-500 uppercase tracking-tighter">CONFIDENCE</span>
            ...
            <div className="pt-4">
                <RadarChart points={result.graphPoints} onLabelClick={(m) => setActiveMetricHelp(m)} lang={currentLang} />
            </div>
        </div>
      </header>
