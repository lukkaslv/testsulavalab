...
export function generateClinicalNarrative(result: AnalysisResult, lang: Lang): ClinicalNarrative {
    ...
    diffExpl = lang === 'ru' ? 
        "Вероятности рассчитаны на основе когнитивной латентности и соматического отклика." : 
        "ალბათობები გამოთვლილია კოგნიტური ლატენტურობისა და სომატური პასუხის საფუძველზე.";
    
    validityExpl = lang === 'ru' ? 
        "Анализ качества сигнала: работайте через легализацию несовершенства." : 
        "სიგნალის ხარისხის ანალიზი: იმუშავეთ არასრულყოფილების ლეგალიზაციის გზით.";

    if (isCompensatoryOverdrive) {
        profileName = labels.armored;
        psychodynamicProfile = lang === 'ru' ? "КЛИНИЧЕСКИЙ ПАРАДОКС: аномально низкая тревога." : "კლინიკური პარადოქსი: ანომალურად დაბალი შფოთვა.";
        deepExpl = lang === 'ru' ? "Система отключает датчики боли." : "სისტემა თიშავს ტკივილის სენსორებს.";
        behaviorExpl = lang === 'ru' ? "Идеальный фасад." : "იდეალური ფასადი.";
        hypoExpl = lang === 'ru' ? "Уязвимость спрятана за функциональностью." : "მოწყვლადობა დამალულია ფუნქციონალურობის მიღმა.";
        interExpl = lang === 'ru' ? "Конфронтация с Ложным Я." : "კონფრონტაცია 'ცრუ მე'-სთან.";
        shadowContract = lang === 'ru' ? "Я дам тебе успех, если ты не будешь лезть мне в душу." : "მე მოგცემ წარმატებას, თუ სულში არ ჩამიძვრები.";
        evolutionGoal = lang === 'ru' ? "Переход к Живому Я." : "გადასვლა 'ცოცხალ მე'-სკენ.";
        evolutionProcess = lang === 'ru' ? "Через потерю контроля." : "კონტროლის დაკარგვის გზით.";
        clinicalStrategy = lang === 'ru' ? ["Легализация пассивности"] : ["პასიურობის ლეგალიზაცია"];
    } else if (state.foundation < 30) {
        profileName = labels.critical;
        psychodynamicProfile = lang === 'ru' ? "Структура 'Голый Провод'." : "სტრუქტურა 'შიშველი სადენი'.";
        deepExpl = lang === 'ru' ? "Режим выживания." : "გადარჩენის რეჟიმი.";
        behaviorExpl = lang === 'ru' ? "Клиент ищет спасателя." : "კლიენტი ეძებს მხსნელს.";
        hypoExpl = lang === 'ru' ? "Ранняя травма привязанности." : "მიჯაჭვულობის ადრეული ტრავმა.";
        interExpl = lang === 'ru' ? "Валидация права на существование." : "არსებობის უფლების ვალიდაცია.";
        shadowContract = lang === 'ru' ? "Усынови меня, чтобы мне не нужно было взрослеть." : "მიშვილე, რომ არ მომიწიოს გაზრდა.";
        evolutionGoal = lang === 'ru' ? "Внутренний контейнер." : "შინაგანი კონტეინერი.";
        evolutionProcess = lang === 'ru' ? "От внешней опоры к внутренней." : "გარე საყრდენიდან შინაგანისკენ.";
        clinicalStrategy = lang === 'ru' ? ["Стабильный сеттинг"] : ["სტაბილური სეტინგი"];
    } else {
        ...
        shadowContract = lang === 'ru' ? "Давай просто поговорим, чтобы ничего не менять." : "მოდი უბრალოდ ვისაუბროთ, რომ არაფერი შევცვალოთ.";
        evolutionGoal = lang === 'ru' ? "Риск живого действия." : "ცოცხალი ქმედების რისკი.";
        evolutionProcess = lang === 'ru' ? "Выход за пределы сценария." : "სცენარის მიღმა გასვლა.";
        clinicalStrategy = lang === 'ru' ? ["Работа с Тенью"] : ["მუშაობა ჩრდილთან"];
    }
    ...
    if (activePatterns.includes('family_loyalty') || state.foundation < 45) {
        systemicVectors.push({ 
            ...
            description: lang === 'ru' ? "Верность родовому дефициту." : "საგვარეულო დეფიციტის ერთგულება.",
            proNote: lang === 'ru' ? "Разрыв контракта с уважением к предкам." : "კონტრაქტის გაწყვეტა წინაპრების პატივისცემით."
        });
    }
    ...
    return {
        ...
        level2: {
            ...
            clinicalHypotheses: lang === 'ru' ? "1. Функциональное отношение к себе." : "1. ფუნქციური დამოკიდებულება საკუთარ თავთან.",
