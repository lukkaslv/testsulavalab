...
import { AnalysisResult, CompatibilityReport, ArchetypeKey, VerdictKey } from '../types';
...
export const CompatibilityEngine = {
  analyzeCompatibility(_b1: AnalysisResult, b2: AnalysisResult): CompatibilityReport {
    return {
      overallScore: 0,
      domainSynergies: [],
      domainConflicts: [],
  ...
  decodeSmartCode(textInput: string): AnalysisResult | null {
    try {
        ...
        if (!isSignatureValid) {
            console.error("BLUE_TEAM_ALERT: CODE_TAMPERING_DETECTED", { sig, calculated: SecurityCore.generateChecksum(data) });
        ...
        // TOLERANCE PATCH: Previous core versions could output values slightly > 100 (up to ~115)
        // Instead of rejecting, we clamp them to maintain accessibility.
        const isSuspiciousOverflow = f > 115 || a > 115 || r > 115 || e > 115;
        if (isSuspiciousOverflow || f < 0 || a < 0 || r < 0 || e < 0) {
             return CompatibilityEngine.createBreachObject();
        }

        // Apply clamping for legacy compatibility
        f = Math.min(100, f);
        a = Math.min(100, a);
        r = Math.min(100, r);
        e = Math.min(100, e);
        ...
        return {
            ...
            phase: 'STABILIZATION',
            archetypeKey: providedArchetypeKey || 'THE_DRIFTER', 
            archetypeMatch: 100,
            archetypeSpectrum: [],
            verdictKey: providedVerdictKey || 'HEALTHY_SCALE', 
